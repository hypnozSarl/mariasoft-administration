def CONTAINER_NAME = "mariasoft_administration"
def ENV_NAME = getEnvName(env.BRANCH_NAME)
def CONTAINER_TAG = getTag(env.BUILD_NUMBER, env.BRANCH_NAME)
def HTTP_PORT = getHTTPPort(env.BRANCH_NAME)
def EMAIL_RECIPIENTS = "hypnozsarl@gmail.com"
    ...
    try {
        ...
        stage('Sonarqube Analysis') {
            withSonarQubeEnv('SonarQubeLocalServer') {
                sh "mvn sonar:sonar -Dsonar.branch.name=${env.BRANCH_NAME} -Dintegration-tests.skip=true -Dmaven.test.failure.ignore=true"
            }
